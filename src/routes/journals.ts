...
type CachedIndex = { ts: number; result: any };
const INDEX_CACHE = new Map<string, CachedIndex>();
const INDEX_CACHE_MS = 30_000;

const VS_JOURNAL_PACKS = new Set<string>([
...
function isAllowedJournalPack(packId: string): boolean {
  return VS_JOURNAL_PACKS.has(String(packId ?? ""));
}

// Minimal shape the router needs from the new store
type JournalsWorldStore = {
  readPacksManifest(worldId: string): Promise<any | null>;
  listJournalTemplates(worldId: string, packId: string): Promise<any | null>;
  readJournalPayload(worldId: string, packId: string, docId: string): Promise<any | null>;
  readDeleteManifest(worldId: string): Promise<any | null>;
};

async function getCachedTemplates(
  worldStore: JournalsWorldStore,
  worldId: string,
  packId: string
) {
  ...
  if (cached && now - cached.ts < INDEX_CACHE_MS) return cached.result;
...
export function makeJournalRouter(deps: CreateAppDeps) {
  const router = Router();
  ...
  router.get("/:worldId/journals/packs", requireWorldMember, async (req, res, next) => {
    try {
      ...
      const out = packs
        .filter((p: any) => isAllowedJournalPack(String(p?.id ?? "")))
        .map((p: any) => ({
          id: String(p?.id ?? ""),
   ...
   */
  router.get(
    "/:worldId/journals/:packId/payloads/:docId",
    requireWorldMember,
    async (req, res, next) => {
      try {
        const worldId = asParamString(req.params.worldId);
        ...
        if (!payload) return res.status(404).json({ ok: false, error: "Journal payload not found" });
