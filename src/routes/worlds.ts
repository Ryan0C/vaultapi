...
type WorldStore = {
  ...
  readUsersMeta(worldId: string): Promise<any | null>;
  readStatusMeta(worldId: string): Promise<any | null>;
  readVaultMeta(worldId: string): Promise<any | null>;
};
...
export function makeWorldsRouter(deps: CreateAppDeps) {
  const router = Router();

  const { worldStore } = deps as unknown as CreateAppDeps & {
  ...
  router.get("/:worldId/status", async (req, res, next) => {
    try {
      ...
      try {
        const world = await worldStore.readWorldMeta(worldId);
        if (!world) return res.status(404).json({ ok: false, error: "world meta not found" });
      } catch (e: any) {
        if (isENOENT(e)) return res.status(404).json({ ok: false, error: "world meta not found" });
        throw e;
      }
