...
import { db, getFoundryCursor,upsertFoundryCursor } from "../services/db.js";
...
function pickAuthUser(req: any) {
  return req.vaultUser ?? req.user ?? null;
}

function pickCharacterContext(req: any) {
  const characterId = asString(req.body?.data?.characterId);
  const characterName =
    asString(req.body?.actorName) ??           // allow top-level actorName
...
function pickEventTs(evt: any): number {
...
function pickEventKind(evt: any): string {
  const msg = evt?.message ?? evt;

  if (msg?.isRoll) return "roll";
  if (Array.isArray(msg?.rolls) && msg.rolls.length) return "roll";

  const mt =
    msg?.flags?.dnd5e?.messageType ??
    msg?.context?.messageType ??
    null;

  if (String(mt ?? "").toLowerCase() === "roll") return "roll";

  return "chat";
}

function pickEventHtml(evt: any): string | null {
  const msg = evt?.message ?? evt;
...
function pickEventSummary(evt: any): string | null {
  const msg = evt?.message ?? evt;
  return asString(
    evt?.text ??
      evt?.content?.text ??
...
function pickActorId(evt: any): string | null {
  const msg = evt?.message ?? evt;
  return asString(
    evt?.actorId ??
      msg?.speaker?.actor ??
...
function pickActorName(evt: any): string | null {
  const msg = evt?.message ?? evt;
  return asString(
    evt?.actorName ??
      msg?.speaker?.alias ??
...
function pickTitle(evt: any): string | null {
  const msg = evt?.message ?? evt;
...
async function importChatShard(args: {
  ...
  let afterTs = Number.isFinite(args.afterTs as any) ? Number(args.afterTs) : 0;
  let pages = 0;

  let imported = 0;
  let lastNextAfterTs: number | null = null;
  ...
  while (pages < maxPages) {
    ...
    for (let i = 0; i < events.length; i++) {
      ...
      if (file && typeof (eventsStore as any).upsertFoundryChatFromVaultFile === "function") {
        (eventsStore as any).upsertFoundryChatFromVaultFile({
          worldId,
          day,
          hour,
          file,
...
export function makeEventsRouter(deps: CreateAppDeps) {
  const router = Router();
