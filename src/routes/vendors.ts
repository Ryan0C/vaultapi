...
function isDm(deps: CreateAppDeps, req: any, worldId: string): boolean {
  if (isApiKeySuperuser(req)) return true;
  const userId = getSessionUserId(req);
  return !!(userId && deps.authStore.isWorldDm(worldId, userId));
}

function requireDm(deps: CreateAppDeps, req: any, worldId: string): { error: true; status: number; message: string } | null {
  if (isApiKeySuperuser(req)) return null;
  const userId = getSessionUserId(req);
  if (!userId) return { error: true, status: 401, message: "Login required" };
  if (!deps.authStore.isWorldDm(worldId, userId)) return { error: true, status: 403, message: "DM access required" };
  return null;
}
...
export function makeVendorsRouter(deps: CreateAppDeps) {
  ...
  router.patch("/:worldId/vendors/:vendorId/toggle", (req, res, next) => {
    try {
      ...
      if (typeof isActive !== "boolean") return bad(res, "isActive (boolean) is required");
  ...
  router.post("/:worldId/vendors/:vendorId/items", (req, res, next) => {
    try {
      ...
      const {
        name, description, imageUrl, foundryItemId,
        priceGold, quantity, maxQuantity,
        restockIntervalSeconds, restockAmount, sortOrder,
      } = req.body;
  ...
  router.post("/:worldId/vendors/:vendorId/buy", (req, res, next) => {
    try {
      ...
      const userId = getSessionUserId(req);
      if (!userId && !isApiKeySuperuser(req)) return bad(res, "Login required", 401);

      const { itemId, quantity, buyerActorId } = req.body;
      if (!itemId) return bad(res, "itemId is required");
      ...
      if (!result.ok) return bad(res, result.error, 400);
      return ok(res, result);
    } catch (err) { next(err); }
  });
