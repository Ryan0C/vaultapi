...
export function getFoundryCursor(worldId: string, day: string, hour: string) {
  const row = db.prepare(`
    SELECT after_ts AS afterTs
    FROM foundry_chat_cursors
    WHERE world_id = ? AND day = ? AND hour = ?
  `).get(String(worldId), String(day), String(hour)) as { afterTs: number } | undefined;

  return row ? { afterTs: Number(row.afterTs ?? 0) } : null;
}

export function upsertFoundryCursor(worldId: string, day: string, hour: string, afterTs: number) {
