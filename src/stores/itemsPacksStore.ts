...
export class ItemsPacksStore {
  ...
  private _latestPackIndexFile = new Map<string, string>();
  ...
  private invalidatePack(worldId: string, packId: string): void {
    const packPrefix = `${worldId}:${packId}:`;
    for (const k of this._cache.keys()) {
      if (k.startsWith(packPrefix)) this._cache.delete(k);
    }
    for (const k of this._searchCache.keys()) {
      if (k.startsWith(packPrefix)) this._searchCache.delete(k);
    }
    for (const k of this._compiledCache.keys()) {
      if (k.startsWith(packPrefix)) this._compiledCache.delete(k);
    }
  ...
  invalidateWorld(worldId: string): void {
    ...
    for (const k of this._latestPackIndexFile.keys()) {
      if (k.startsWith(prefix)) this._latestPackIndexFile.delete(k);
    }
  }
  ...
  async readLatestPackIndex(worldId: string, packId: string): Promise<any | null> {
    ...
    const cached = this._cacheGet(cacheKey);
    if (cached !== undefined && prevLatest === latest) return cached;
  ...
  async readBuilderChoices(worldId: string): Promise<{
    ...
    if (cached && cached.exp > Date.now() && cached.signature === signature) return cached.value;
