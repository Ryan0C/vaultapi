...
describe("vault-api", () => {
    ...
    it("GET /worlds returns worlds array (shape)", async () => {
        ...
        for (const w of res.body.worlds) {
    ...
    it("World meta / actors / chat routes behave (shape)", async () => {
        ...
        expect(actorsRes.body).toEqual(expect.objectContaining({
        ...
        if (actorId) {
            ...
            expectJson(actorRes);
            expect(actorRes.body).toEqual(expect.objectContaining({
                ok: true,
        ...
        expectJson(daysRes);
        expect(daysRes.body).toEqual(expect.objectContaining({
            ok: true,
        ...
        expectJson(hoursRes);
        expect(hoursRes.body).toEqual(expect.objectContaining({
            ok: true,
        ...
        // events list
        const eventsRes = await auth(request(app).get(`/worlds/${worldId}/chat/events/${day}/${hour}?afterTs=0&limit=50`));
        ...
        expectJson(oneRes);
        expect(oneRes.body).toEqual(expect.objectContaining({
            ok: true,
    ...
    it("GET /worlds/:worldId/chat/events/:day/:hour/:file returns 400 for invalid filename", async () => {
        ...
        const res = await auth(request(app).get(`/worlds/${worldId}/chat/events/2026-02-11/22/not-json.txt`));
        expectJson(res);
