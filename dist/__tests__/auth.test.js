...
async function createAndLoginUser(app, suffix) {
...
describe("auth + authz (session + api key)", () => {
    ...
    it("player actor list only includes linked character actors", async () => {
        ...
        const actorIds = res.body.actors.map((actor) => String(actor?.id ?? ""));
        expect(actorIds).toContain("actorA");
        expect(actorIds).not.toContain("actorB");
        expect(res.body.actors.every((actor) => String(actor?.type ?? "").toLowerCase() === "character")).toBe(true);
    });
