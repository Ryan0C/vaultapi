...
describe("ItemsPacksStore builder choices cache invalidation", () => {
    ...
    it("refreshes builder choices immediately when a pack index is re-exported", async () => {
        ...
        await writeJson(path.join(packDir, "index.100.alpha.json"), packIndex([{ _id: "fighter", name: "Fighter", type: "class", system: {} }]));
        const store = new ItemsPacksStore({ foundryDataRoot: root });
        const first = await store.readBuilderChoices(worldId);
        expect(first.classes.map((x) => x.name)).toContain("Fighter");
        await writeJson(path.join(packDir, "index.200.beta.json"), packIndex([{ _id: "wizard", name: "Wizard", type: "class", system: {} }]));
        const second = await store.readBuilderChoices(worldId);
        const names = second.classes.map((x) => x.name);
    ...
    it("includes newly added pack directories without waiting for pack-id cache TTL", async () => {
        ...
        await writeJson(path.join(base, "pack-a", "index.100.alpha.json"), packIndex([{ _id: "class-1", name: "Rogue", type: "class", system: {} }]));
        const store = new ItemsPacksStore({ foundryDataRoot: root });
        const first = await store.readBuilderChoices(worldId);
        expect(first.classes.map((x) => x.name)).toEqual(["Rogue"]);
        await writeJson(path.join(base, "pack-b", "index.110.beta.json"), packIndex([{ _id: "species-1", name: "Elf", type: "species", system: {} }]));
        const second = await store.readBuilderChoices(worldId);
        expect(second.classes.map((x) => x.name)).toEqual(["Rogue"]);
        expect(second.species.map((x) => x.name)).toContain("Elf");
    });
});
//# sourceMappingURL=itemsPacksStore.spec.js.map