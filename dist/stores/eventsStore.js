...
function deriveFoundryKind(msg) {
...
export class EventsStore {
    ...
    upsertFoundryChatFromVaultFile(args) {
        ...
        if (!day || !hour || !file)
            return { ok: false, error: "Missing shard identity (day/hour/file)" };
        const raw = args.raw ?? {};
        ...
        const tsRaw = toInt(msg?.timestamp, 0) ||
            toInt(raw?.ts, 0) ||
            Date.now();
        ...
        const actorName = msg?.speaker?.alias != null ? String(msg.speaker.alias) :
        ...
        const summary = html ? stripHtmlToText(html) :
            msg?.content != null ? String(msg.content) :
                null;
        const groupId = msg?.id != null ? String(msg.id) :
    ...
    listEvents(args) {
        ...
        const where = [`world_id = @worldId`];
        const params = { worldId, limit };
