...
export function makeRequireDocPickerAccess(deps) {
    const { vault, authStore } = deps;
    return async function requireDocPickerAccess(req, _res, next) {
        try {
            const anyReq = req;
            ...
            if (!picker?.enabled)
                return next(forbidden("Picker access is disabled for this world"));
            const playerAccess = String(picker.playerAccess ?? "dmOnly");
            if (playerAccess === "dmOnly")
                return next(forbidden("Picker access is DM-only for this world"));
...
//# sourceMappingURL=docPolicy.js.map