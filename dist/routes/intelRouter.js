...
async function requireIntelWriteAccess(deps, req, worldId) {
...
function normalizeViewerRole(roleRaw) {
    const s = cleanStr(roleRaw);
    return s === "gm" ? "gm" : "player";
}
async function requireIntelDiscoverAccess(deps, req, worldId) {
    // superuser ok
    if (isApiKeySuperuser(req))
        return { role: "gm" };
    const userId = getSessionUserId(req);
    ...
    return { role: deps.authStore.isWorldDm(worldId, userId) ? "gm" : "player", userId };
}
export function makeIntelRouter(deps) {
    const router = Router();
    ...
    router.get("/:worldId/intel", requireWorldMember, async (req, res, next) => {
        try {
            ...
            const list = intelStore.listIntel({
                worldId,
                kind: cleanStr(req.query?.kind) ? cleanStr(req.query?.kind) : undefined,
                scope: cleanStr(req.query?.scope) ? normalizeScope(req.query?.scope) : undefined,
    ...
    router.get("/:worldId/intel/discovered", requireWorldMember, async (req, res, next) => {
        try {
            const { worldId } = req.params;
            const viewerActorId = cleanStr(req.query?.viewerActorId ?? req.query?.actorId) || undefined;
            const list = intelStore.listIntel({
                worldId,
                kind: cleanStr(req.query?.kind) ? cleanStr(req.query?.kind) : undefined,
                viewerRole: "player",
    ...
    router.get("/:worldId/intel/:intelId", requireWorldMember, async (req, res, next) => {
        try {
            ...
            if (!intel)
                return bad(res, "Intel not found", 404);
    ...
    router.patch("/:worldId/intel/:intelId", requireWorldMember, async (req, res, next) => {
        try {
            ...
            if ("discovered_at" in patch && !("discoveredAt" in patch))
                patch.discoveredAt = patch.discovered_at;
            if ("discovered_by_actor_id" in patch && !("discoveredByActorId" in patch))
                patch.discoveredByActorId = patch.discovered_by_actor_id;
            // Normalize types
            if ("scope" in patch)
                patch.scope = normalizeScope(patch.scope);
            if ("actorId" in patch)
                patch.actorId = cleanStr(patch.actorId) || null;
            if ("title" in patch)
                patch.title = cleanStr(patch.title);
            if ("kind" in patch)
                patch.kind = cleanStr(patch.kind);
            if ("discoveredAt" in patch)
                patch.discoveredAt = toIsoOrNull(patch.discoveredAt);
            if ("discoveredByActorId" in patch)
                patch.discoveredByActorId = cleanStr(patch.discoveredByActorId) || null;
            // Normalize list fields
            if ("tags" in patch && patch.tags != null && !Array.isArray(patch.tags))
                return bad(res, "tags must be an array", 400);
    ...
    router.post("/:worldId/intel/:intelId/upload", requireWorldMember, upload.single("file"), async (req, res, next) => {
        try {
            const anyReq = req;
            const { worldId: worldIdRaw, intelId } = req.params;
            await requireIntelWriteAccess(deps, anyReq, worldIdRaw);
            if (!deps.foundryDataRoot)
                return bad(res, "FOUNDRY_DATA_ROOT not configured", 500);
            if (!req.file)
                return bad(res, "Missing file", 400);
    ...
    router.post("/:worldId/intel/:intelId/attachments", requireWorldMember, async (req, res, next) => {
        try {
            ...
            if (!mediaId)
                return bad(res, "Missing mediaId");
    ...
    router.post("/:worldId/intel/:intelId/attachments/reorder", requireWorldMember, async (req, res, next) => {
        try {
            ...
            if (!orderedIds)
                return bad(res, "orderedIds must be an array");
...
//# sourceMappingURL=intelRouter.js.map