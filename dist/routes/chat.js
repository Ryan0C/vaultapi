...
export function makeChatRouter(deps) {
    const router = Router();
    ...
    router.get("/:worldId/chat/recent", requireWorldMember, async (req, res, next) => {
        try {
            ...
            const chatDir = path.resolve(String(dataRoot), `worlds/${worldId}/vaultsync/exports/chat`);
            let dirFiles;
            try {
    ...
    router.get("/:worldId/chat/manifests/:day/:hour", requireWorldMember, async (req, res, next) => {
        try {
            ...
            if (!manifest)
                return res.status(404).json({ ok: false, error: "Shard manifest not found" });
    ...
    router.get("/:worldId/chat/events/:day/:hour", requireWorldMember, async (req, res, next) => {
        try {
            ...
            const { events, nextAfterTs } = await vault.listChatEvents(worldId, { day, hour }, { afterTs, limit });
            res.json({
                ok: true,
                day,
                hour,
    ...
    router.get("/:worldId/chat/events/:day/:hour/:file", requireWorldMember, async (req, res, next) => {
        try {
            ...
            const dir = vault.chatShardDir(worldId, { day, hour });
            const rel = `${dir}/${file}`;
            const exists = await vault.exists(rel);
            if (!exists)
                return res.status(404).json({ ok: false, error: "Event not found" });
            const evt = await vault.readJson(rel);
