...
export function makeImportsRouter(deps) {
    const router = Router();
    const { authStore, importsStore } = deps;
    ...
    router.post("/:worldId/imports", requireWorldMember, async (req, res, next) => {
        try {
            ...
            const result = await importsStore.createImport(worldId, {
                type,
                entityType: req.body?.entityType != null ? String(req.body.entityType) : undefined,
                entityId: req.body?.entityId != null ? String(req.body.entityId) : null,
                payload,
                ...
                requestId: req.body?.requestId != null ? String(req.body.requestId) : undefined,
            });
            if (!result.ok) {
