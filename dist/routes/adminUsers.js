...
const WORLD_ROLES = new Set(["dm", "player", "observer"]);
const ACTOR_PERMS = new Set(["owner", "editor", "viewer"]);
function asNonEmptyString(v) {
    const s = String(v ?? "").trim();
    return s ? s : null;
}
function asWorldRole(v) {
    const s = String(v ?? "").trim().toLowerCase();
    return WORLD_ROLES.has(s) ? s : "player";
}
function asActorPerm(v) {
    const s = String(v ?? "").trim().toLowerCase();
    return ACTOR_PERMS.has(s) ? s : "owner";
}
function asMinutes(v, fallback = 60) {
    const n = Number(v);
...
export function makeAdminUsersRouter(deps) {
    const router = Router();
    ...
    function assertUserExists(res, userId) {
    ...
    router.post("/users/:userId/reset", requireSuperadmin, (req, res) => {
        ...
        if (!assertUserExists(res, userId))
            return;
        const createdBy = resolveCreatedBy(req);
    ...
    router.post("/users/:userId/worlds", requireSuperadmin, (req, res) => {
        ...
        if (!worldId)
            return badRequest(res, "worldId is required");
    ...
    router.post("/users/:userId/actors", requireSuperadmin, (req, res) => {
        ...
        if (!worldId || !actorId)
            return badRequest(res, "worldId and actorId are required");
    ...
    router.delete("/users/:userId/actors/:worldId/:actorId", requireSuperadmin, (req, res) => {
        ...
        if (!worldId)
            return badRequest(res, "worldId is required");
        if (!actorId)
            return badRequest(res, "actorId is required");
        if (!assertUserExists(res, userId))
            return;
