...
async function getCachedPackEntries(vault, worldId, packId) {
...
function expandKind(kind) {
    ...
    if (!k)
        return []; // means "any allowed kind"
    // If you ask for feats, also search likely “feature” packs.
    if (k === "feats")
        return ["feats", "classfeatures", "features", "options"];
    // Items might be split across items/equipment/tradegoods
    if (k === "items")
        return ["items", "equipment", "equipment24", "tradegoods"];
    return [k];
}
function tokenize(s) {
    return s
...
function packIdFromCompendiumUuid(uuid) {
...
export function makeDocsRouter(deps) {
    const router = Router();
    const { vault, authStore } = deps;
    ...
    router.get("/:worldId/search", requireWorldMember, requireDocPickerAccess, async (req, res, next) => {
        try {
            ...
            const kindParam = String(req.query?.kind ?? "").trim() || null;
            ...
            const allowedKinds = Array.isArray(picker.allowedKinds) ? picker.allowedKinds : [];
            ...
            if (kindList.length && allowedKinds.length) {
                ...
                if (!okAny)
                    return next(forbidden("Search kind not permitted by policy"));
            }
            ...
            const playerPackAllowlist = Array.isArray(picker.playerPackAllowlist) ? picker.playerPackAllowlist : [];
            // Which packs can we search?
            const searchablePacks = packs.filter((p) => {
                const pKind = String(p?.kind ?? "");
                if (allowedKinds.length && !allowedKinds.includes(pKind))
                    return false;
                if (kindList.length && !kindList.includes(pKind))
                    return false;
                if (isDm)
                    return true;
                if (!picker.enabled)
    ...
    router.get("/:worldId/docs", requireWorldMember, async (req, res, next) => {
        try {
            ...
            if (!uuid)
                return res.status(400).json({ ok: false, error: "Missing uuid query param" });
            const anyReq = req;
            ...
            if (!isApiKeySuperuser(anyReq)) {
                ...
                if (allowed && !allowed.has(uuid))
                    return next(forbidden("Document access denied"));
            }
        ...
        try {
            ...
            if (isDm) {
                ...
                if (permitted.length !== requested.length)
                    return next(forbidden("Some requested documents are not permitted"));
            }
            else if (mode === "picker") {
                ...
                const picker = policy?.docPolicy?.picker ?? {};
                if (!picker?.enabled)
                    return next(forbidden("Picker access disabled"));
                const playerAccess = String(picker.playerAccess ?? "dmOnly");
                const allowPacks = Array.isArray(picker.playerPackAllowlist) ? picker.playerPackAllowlist : [];
                if (playerAccess === "dmOnly")
                    return next(forbidden("Picker access is DM-only"));
                ...
                if (permitted.length !== requested.length)
                    return next(forbidden("Some requested documents are outside the allowed packs"));
            }
            else {
