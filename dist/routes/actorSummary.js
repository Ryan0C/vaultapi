export function asParamString(value) {
    if (Array.isArray(value))
        return String(value[0] ?? "").trim();
    return String(value ?? "").trim();
}
export function unwrapActorSnapshot(actor, actorIdHint) {
...
export function actorSpecies(actor) {
...
export function isCharacterSnapshot(actor) {
    const unwrapped = unwrapActorSnapshot(actor);
    return String(unwrapped?.type ?? "").toLowerCase() === "character";
}
//# sourceMappingURL=actorSummary.js.map