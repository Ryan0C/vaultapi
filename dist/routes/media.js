...
function safeResolveFromRoots(roots, rel) {
...
// media.ts
export function makeMediaRouter(deps) {
    const router = Router();
    ...
    router.get(/.*/, requireUser, (req, res, next) => {
        try {
            ...
            if (!dataRoot)
                return next(forbidden("FOUNDRY_DATA_ROOT not configured"));
            ...
            if (publicRoot && rel.startsWith("icons/"))
                roots.push(publicRoot);
            // Usual data roots
            roots.push(dataRoot, path.join(dataRoot, "systems"), path.join(dataRoot, "modules"), path.join(dataRoot, "worlds"));
            // As a fallback, also allow publicRoot for anything
            if (publicRoot && !roots.includes(publicRoot))
                roots.push(publicRoot);
            const abs = safeResolveFromRoots(roots, rel);
            if (!abs)
                return res.status(404).json({ ok: false, error: "Media not found", rel, roots });
