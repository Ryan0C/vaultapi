...
function getUserEnvelope(deps, req, worldId) {
    // try to include both appUserId + foundryUserId if you have a link table
    const appUserId = getSessionUserId(req) ?? undefined;
    let foundryUserId = undefined;
    if (appUserId) {
        const links = deps.authStore.listUserWorldLinks(appUserId);
        const link = links.find((l) => String(l.worldId) === String(worldId));
        if (link?.foundryUserId)
            foundryUserId = String(link.foundryUserId);
    }
    return { appUserId, foundryUserId };
}
async function requireCommandWriteAccess(deps, req, worldId) {
    ...
    if (!enabled)
        throw forbidden("Commands are disabled by world policy");
    const playerAccess = String(cmdPolicy?.playerAccess ?? "dmOnly");
    if (playerAccess !== "allow")
        throw forbidden("Commands are DM-only for this world");
    return { isDm: false };
}
function cleanStr(v) {
...
export function makeCommandsRouter(deps) {
    const router = Router();
    ...
    router.post("/:worldId/commands", requireWorldMember, requireCommandAccess, async (req, res, next) => {
        try {
            ...
            if (!type)
                return res.status(400).json({ ok: false, error: "Missing/invalid command type" });
