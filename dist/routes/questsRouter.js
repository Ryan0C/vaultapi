...
async function requireQuestWriteAccess(deps, req, worldId) {
...
function normalizeRestricted(input) {
    if (input == null)
        return null;
    ...
    if (typeof input === "object") {
        return {
            ...
            userIds: Array.isArray(input.userIds) ? input.userIds.map(String) : [],
        };
    }
    return null;
}
export function makeQuestRouter(deps) {
    const router = Router();
    ...
    router.get("/:worldId/quests/assigned", requireWorldMember, async (req, res, next) => {
        try {
            ...
            const scope = (cleanStr(req.query?.scope) || "party");
            ...
            if (scope === "actor") {
                if (!actorId)
                    return bad(res, "actorId is required when scope=actor");
            ...
            return ok(res, {
                ...
                quests: out.map((r) => ({
                    ...
                    quest: {
                        ...
                        status: r.status,
                        reward: (() => { try {
                            return JSON.parse(r.reward_json ?? "null");
                        }
                        catch {
                            return null;
                        } })(),
                        ...
                        restricted: (() => {
                            try {
                                return JSON.parse(r.restricted_json ?? "null");
                            }
                            catch {
                                return null;
                            }
    ...
    router.get("/:worldId/quests/:questId", requireWorldMember, async (req, res, next) => {
        try {
            ...
            if (!quest)
                return bad(res, "Quest not found", 404);
    ...
    router.patch("/:worldId/quests/:questId", requireWorldMember, async (req, res, next) => {
        try {
            ...
            if ("available_from" in patch && !("availableFrom" in patch))
                patch.availableFrom = patch.available_from;
            if ("available_until" in patch && !("availableUntil" in patch))
                patch.availableUntil = patch.available_until;
            if ("auto_fail_on_expire" in patch && !("autoFailOnExpire" in patch))
                patch.autoFailOnExpire = patch.auto_fail_on_expire;
            // Normalize types
            if ("availableFrom" in patch)
                patch.availableFrom = toIsoOrNull(patch.availableFrom);
            if ("availableUntil" in patch)
                patch.availableUntil = toIsoOrNull(patch.availableUntil);
            if ("autoFailOnExpire" in patch)
                patch.autoFailOnExpire = toBool01(patch.autoFailOnExpire, 0) === 1;
            // normalize reward
            if ("reward_json" in patch && !("reward" in patch))
                patch.reward = patch.reward_json;
            // normalize category (optional; either works)
            if ("category" in patch)
                patch.category = cleanStr(patch.category) || null;
            // normalize restricted (support legacy array OR object)
            if ("restricted" in patch)
                patch.restricted = normalizeRestricted(patch.restricted);
            // normalize tags_json -> tags
            if ("tags_json" in patch && !("tags" in patch)) {
                try {
                    patch.tags = JSON.parse(patch.tags_json ?? "[]");
                }
                catch {
                    patch.tags = [];
                }
            ...
            if (!out.ok)
                return bad(res, out.error ?? "Update failed", 400);
    ...
    router.post("/:worldId/assignments/:assignmentId/timer/resume", requireWorldMember, async (req, res, next) => {
        try {
            ...
            if (!out.ok)
                return bad(res, out.error ?? "Resume failed", 400);
            return ok(res, { worldId, assignmentId, ...out });
        }
        catch (err) {
            next(err);
    ...
    router.post("/:worldId/quests/:questId/assign", requireWorldMember, async (req, res, next) => {
        try {
            ...
            const { worldId, questId } = req.params;
            await requireQuestWriteAccess(deps, anyReq, worldId);
            const scope = (cleanStr(req.body?.scope) || "party");
            const actorId = cleanStr(req.body?.actorId) || null;
            const durationSeconds = req.body?.durationSeconds != null ? toInt(req.body.durationSeconds, 0) : null;
    ...
    router.post("/:worldId/assignments/:assignmentId/timer/start", requireWorldMember, async (req, res, next) => {
        try {
            ...
            if (!durationSeconds || durationSeconds < 1)
                return bad(res, "durationSeconds must be >= 1");
    ...
    router.get("/:worldId/assignments/:assignmentId/objectives", requireWorldMember, async (req, res, next) => {
        try {
            ...
            if (!assignment)
                return bad(res, "Assignment not found", 404);
    ...
    router.post("/:worldId/quests/:parentQuestId/children", requireWorldMember, async (req, res, next) => {
        try {
            ...
            if (!childQuestId)
                return bad(res, "Missing childQuestId");
            ...
            if (!out.ok)
                return bad(res, out.error ?? "Link failed", 400);
    ...
    router.post("/:worldId/quest-chains", requireWorldMember, async (req, res, next) => {
        try {
            ...
            const { worldId } = req.params;
            await requireQuestWriteAccess(deps, anyReq, worldId);
            if (typeof questStore.createChain !== "function") {
                return bad(res, "Quest chains not implemented in questStore yet", 501);
            }
            const title = cleanStr(req.body?.title);
            if (!title)
                return bad(res, "Missing title");
            const out = questStore.createChain({
                worldId,
                title,
                summary: req.body?.summary ?? undefined,
                tags: Array.isArray(req.body?.tags) ? req.body.tags : undefined,
    ...
    router.get("/:worldId/quest-chains/:chainId", requireWorldMember, async (req, res, next) => {
        try {
            ...
            if (typeof questStore.getChainGraph !== "function") {
                return bad(res, "Quest chains not implemented in questStore yet", 501);
            }
            const graph = questStore.getChainGraph({ worldId, chainId });
    ...
    router.patch("/:worldId/quest-chains/:chainId", requireWorldMember, async (req, res, next) => {
        try {
            ...
            const { worldId, chainId } = req.params;
            await requireQuestWriteAccess(deps, anyReq, worldId);
            if (typeof questStore.updateChain !== "function") {
                return bad(res, "Quest chains not implemented in questStore yet", 501);
            }
            const patch = (req.body?.patch ?? req.body ?? {});
            const out = questStore.updateChain({
                worldId,
                chainId,
                patch,
    ...
    router.delete("/:worldId/quest-chains/:chainId", requireWorldMember, async (req, res, next) => {
        try {
            ...
            const { worldId, chainId } = req.params;
            await requireQuestWriteAccess(deps, anyReq, worldId);
            if (typeof questStore.deleteChain !== "function") {
                return bad(res, "Quest chains not implemented in questStore yet", 501);
            }
            const out = questStore.deleteChain({ worldId, chainId });
    ...
    router.get("/:worldId/quest-packs/export", requireWorldMember, async (req, res, next) => {
        try {
            ...
            const questExternalIdById = new Map();
            for (const q of quests)
                questExternalIdById.set(q.id, String(q.id));
            const objectives = [];
            ...
            const chains = [];
            if (includeChains) {
                const cs = questStore.listChains(worldId);
                for (const c of cs) {
                    const graph = questStore.getChainGraph({ worldId, chainId: c.id });
                    chains.push({
    ...
    router.post("/:worldId/quest-packs/import", requireWorldMember, async (req, res, next) => {
        try {
            ...
            const questsIn = Array.isArray(data?.quests) ? data.quests : [];
            const objectivesIn = Array.isArray(data?.objectives) ? data.objectives : [];
            const linksIn = Array.isArray(data?.links) ? data.links : [];
            const chainsIn = Array.isArray(data?.chains) ? data.chains : [];
            if (!questsIn.length)
                return bad(res, "Pack missing data.quests[]");
            const createdBy = getSessionUserId(anyReq) ?? undefined;
            ...
            for (let i = 0; i < questsIn.length; i++) {
                ...
                if (!title)
                    return bad(res, `Quest missing title (externalId=${externalId})`);
                const created = questStore.createQuest({
                    worldId,
                    title,
                ...
                const patch = {};
                if (q.status != null)
                    patch.status = q.status;
                if (q.priority != null)
                    patch.priority = toInt(q.priority, 0);
            ...
            for (const o of objectivesIn) {
                ...
                if (!questId)
                    continue; // skip unknown mapping for now
    ...
    router.post("/:worldId/quest-chains/:chainId/nodes", requireWorldMember, async (req, res, next) => {
        try {
            ...
            if (!questId)
                return bad(res, "Missing questId");
    ...
    router.patch("/:worldId/quest-chains/:chainId/nodes/:questId", requireWorldMember, async (req, res, next) => {
        try {
            ...
            await requireQuestWriteAccess(deps, anyReq, worldId);
            if (typeof questStore.upsertChainNode !== "function") {
    ...
    router.post("/:worldId/quest-chains/:chainId/edges", requireWorldMember, async (req, res, next) => {
        try {
            ...
            const { worldId, chainId } = req.params;
            await requireQuestWriteAccess(deps, anyReq, worldId);
            if (typeof questStore.upsertChainEdge !== "function") {
            ...
            if (!fromQuestId || !toQuestId)
                return bad(res, "Missing fromQuestId/toQuestId");
            const edge = questStore.upsertChainEdge({
                worldId,
                chainId,
                fromQuestId,
                toQuestId,
    ...
    router.patch("/:worldId/quest-chains/:chainId/edges/:edgeId", requireWorldMember, async (req, res, next) => {
        try {
            ...
            await requireQuestWriteAccess(deps, anyReq, worldId);
            if (typeof questStore.updateChainEdge !== "function") {
                return bad(res, "questStore.updateChainEdge not implemented", 501);
            }
            const patch = (req.body?.patch ?? req.body ?? {});
            // normalize snake_case -> camelCase inputs
            if ("gate_mode" in patch && !("gateMode" in patch))
                patch.gateMode = patch.gate_mode;
            if ("condition_json" in patch && !("condition" in patch))
                patch.condition = patch.condition_json;
            if ("auto_assign" in patch && !("autoAssign" in patch))
                patch.autoAssign = patch.auto_assign;
            if ("delay_seconds" in patch && !("delaySeconds" in patch))
                patch.delaySeconds = patch.delay_seconds;
            if ("ui_json" in patch && !("ui" in patch))
                patch.ui = patch.ui_json;
            if ("sort_order" in patch && !("sortOrder" in patch))
                patch.sortOrder = patch.sort_order;
            const cleanPatch = {};
            if ("gateMode" in patch)
                cleanPatch.gateMode = cleanStr(patch.gateMode) || "all";
            if ("condition" in patch)
                cleanPatch.condition = patch.condition ?? null;
            if ("autoAssign" in patch)
                cleanPatch.autoAssign = toBool01(patch.autoAssign, 0) === 1;
            if ("delaySeconds" in patch)
                cleanPatch.delaySeconds = toInt(patch.delaySeconds, 0);
            if ("ui" in patch)
                cleanPatch.ui = patch.ui ?? null;
            if ("sortOrder" in patch)
                cleanPatch.sortOrder = toInt(patch.sortOrder, 0);
            const out = questStore.updateChainEdge({
                worldId,
                chainId,
                edgeId,
                patch: cleanPatch,
    ...
    router.delete("/:worldId/quest-chains/:chainId/edges/:edgeId", requireWorldMember, async (req, res, next) => {
        try {
            ...
            const { worldId, chainId, edgeId } = req.params;
            await requireQuestWriteAccess(deps, anyReq, worldId);
            if (typeof questStore.deleteChainEdge !== "function") {
                return bad(res, "questStore.deleteChainEdge not implemented", 501);
            }
            const out = questStore.deleteChainEdge({ worldId, chainId, edgeId });
...
//# sourceMappingURL=questsRouter.js.map